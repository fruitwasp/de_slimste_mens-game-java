...
public class TurnIngelijstController extends TurnController {
	
	private ArrayList<String> correctAnswers;
	
	public TurnIngelijstController(MainController mainController) {
	...
	public void beginTurn(Game game, Round round, Turn turn) {
		...
		if (turn.getQuestion() == null) {
			Question question = mainController.getGameController().fetchRandomQuestion("ingelijst", turn.getUsername());
			
			turn.setQuestion(question);
			
			databaseController.insertQuery("UPDATE beurt SET vraag_id = " + question.getId() + " WHERE spel_id = " + game.getId() + " AND rondenaam = 'ingelijst' AND beurt_id = " + turn.getId());
		}	
	...
	public void onAttemptSubmitted(String attempt) {		
		...
		databaseController.insertQuery(""
			...
			+ "VALUES(" + game.getId() + ", 'ingelijst', " + turn.getId() + ", " + nextAttemptId + ", '" + attempt + "', " + getCurrentScore() + ")");
